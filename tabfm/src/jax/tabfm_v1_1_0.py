...
"""TabFM v1.1.0 Model Release.

This module provides simple access to the TabFM v1.1.0 model architecture and
...
# Hugging Face repository ID for TabFM v1.1.0
HF_REPO_ID = "google/tabfm-1.1.0-jax"


@dataclass(frozen=True)
class Config:
  """Hardcoded architecture configuration for TabFM v1.1.0."""
...
class ClassificationConfig(Config):
  """Architecture configuration for TabFM v1.1.0 classification model."""
...
class RegressionConfig(Config):
  """Architecture configuration for TabFM v1.1.0 regression model."""
...
class TabFM_HF(TabFM, ModelHubMixin):
  ...
  def _from_pretrained(
    ...
    if os.path.isdir(model_id):
      ...
      logging.info(
          "Downloading TabFM v1.1.0 JAX %s weights from Hugging Face...",
          model_type,
      )
...
def load(
  ...
  """Loads the TabFM v1.1.0 JAX model with pre-trained weights.

  If `checkpoint_path` is not provided, downloads weights from Hugging Face
  (google/tabfm-1.1.0-jax), fetching only the requested model_type subfolder.
